...
NebulaGraph is designed and implemented for NVMe SSD. All default parameters are optimized for the SSD devices and require extremely high IOPS and low latency. Local SSD devices, or AWS Provisioned IOPS SSD equivalence is suggested to use.
...
3. (Not required) If the CMake version is incorrect, visit the CMake official website to install the required version.

4. (Not required) If the G++ version is incorrect, visit the G++ official website or follow the instructions below to to install the required version.
...
| graphd (the query engine service process) | 1 or more        |
  ...
  - For a single-machine test environment, you can deploy 1 metad, 1 storaged, and 1 graphd processes in the machine.
...
| Process                                   | Suggested number | Note |
| ----------------------------------------- | ---------------- | --- |
| metad (the metadata service process)      | 3                |Each metad process automatically creates and maintains a replica of the metadata. Usually, you need to deploy three metad processes and only three|
| storaged (the storage service process)    | 3 or more        |The number of storaged processes does not affect the number of graph space replicas|
